...
В `public/plan/` сохранён подготовительный сайт поездки из бывшего отдельного
репозитория `Balkhash_repo`: маршрут, снаряжение и подборка книг. После сборки
он доступен по адресу `/balhash/plan/` и связан с основным сайтом через меню.
...
только скачивают, лежит в GitHub Releases.**

| | Где | Почему |
| --- | --- | --- |
| Фото, 172 шт, 102 МБ | репозиторий | без них нечего показывать |
| Превью, 16 МБ | репозиторий | обложки в сетке |
| Фильмы, по ~90 МБ | репозиторий | см. ниже |
| Архивы `.zip`, 210 МБ | релиз `films-v1` | их качают, а не смотрят |

Фильмы сначала лежали в релизе: это снимало ограничение git на 100 МиБ и
позволяло не жать их вовсе. Но GitHub отдаёт вложения релиза как
`application/octet-stream` с `content-disposition: attachment`. Chrome сам
догадывается, что внутри видео, а Safari отказывается проигрывать такое
встроенным плеером и показывает чёрный прямоугольник с перечёркнутым play.
Поэтому фильмы ужаты под лимит и раздаются сайтом как `video/mp4`.

Архивы остались в релизе: их скачивают, а не смотрят, и `octet-stream` там
уместен. После пересборки перезалить:
